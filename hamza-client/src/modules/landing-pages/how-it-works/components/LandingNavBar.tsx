...
import {
    ...
    DropdownMenuItem,
    DropdownMenuTrigger,
} from '@modules/landing-pages/how-it-works/components/ui/dropdown-menu';
...
export default function LandingNavBar({
    ...
    const navLinks = [
        ...
        { href: '#crypto', label: 'Accepted Crypto' },
        ...
        <nav className="relative">
            ...
            <div className="relative z-10 max-w-[1200px] mx-auto px-4">
                <div className="flex items-center h-16 sm:h-18 lg:h-20">
                    ...
                    <div className="flex-shrink-0 mr-2 sm:mr-4 lg:mr-8">
                        <Link href="/" className="flex-shrink-0">
                            <svg
                                ...
                                className="w-16 h-6 sm:w-20 sm:h-7 lg:w-20 lg:h-8"
                            >
                                <path
                    ...
                    {/* Right Side Items */}
                    <div className="ml-auto flex items-center space-x-1 sm:space-x-2 lg:space-x-3">
                        <div className="hidden md:flex items-center space-x-6">
                            {navLinks.map((link) => (
                                <a
                                    key={link.label}
                                    href={link.href}
                                    className="hover:text-green-400 transition-colors duration-300 px-3 py-2 rounded-md text-sm font-medium"
                                >
                                    {link.label}
                                </a>
                            ))}
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent
                                align="end"
                                className="bg-[#121212] text-white border-gray-800"
                            >
                                <DropdownMenuItem
                                    onClick={() => onSelectLanguage('EN')}
                                    className="hover:bg-green-400 focus:bg-green-400 hover:text-black focus:text-black"
                                >
                                    English
                                </DropdownMenuItem>
                                <DropdownMenuItem
            ...
            {isMobileMenuOpen && (
                <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-start justify-end md:hidden">
                    <div className="bg-black w-72 sm:w-80 h-full overflow-y-auto border-l border-gray-800">
                        <div className="p-4 space-y-2">
                            {navLinks.map((link) => (
                                <a
                                    ...
                                    className="hover:bg-gray-800 hover:text-white block px-3 py-2 rounded-md text-base font-medium"
                                >
                                    {link.label}
                                </a>
                            ))}
                            <Button
                                variant="outline"
                                className="w-full mt-4 border-green-400 text-green-400 hover:bg-green-400 hover:text-black"
                                asChild
                                onClick={() => setIsMobileMenuOpen(false)}
                            >
