...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    await handler.handle(async () => {
        ...
        const { cart_id, create, save } = handler.inputParams;
        ...
        if (cart) {
            if (handler.customerId?.length) {
                if (cart.customer_id != handler.customerId)
                    return handler.returnStatusWithMessage(
                        401,
                        `Cart ${cart_id} belongs to customer ${cart.customer_id}, not ${handler.customerId}`
                    );
            } else {
                ...
                if (cart.customer_id?.length)
                    return handler.returnStatusWithMessage(
                        401,
                        `Cart ${cart_id} belongs to customer ${cart.customer_id}, not anonymous`
                    );
            }
        }
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    await handler.handle(async () => {
        ...
        //add customer id
        if (data)
            data.customer_id = handler?.customerId?.length
                ? handler.customerId
                : null;
        else
            data = {
                customer_id: handler?.customerId?.length
                    ? handler.customerId
                    : null,
            };
...
export const PUT = async (req: MedusaRequest, res: MedusaResponse) => {
    ...
    await handler.handle(async () => {
        //validate
        if (!handler.requireParams(['cart_id', 'data'])) return;
